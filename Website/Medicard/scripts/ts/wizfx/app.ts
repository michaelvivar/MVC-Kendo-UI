    ...
    if (self != top) {
        ...
        window.onresize = _.debounce(() => {
            $('.k-context-menu').each(function () {
                let $cm = $(this).data(wg.config.kendo.contextmenu);
                if ($cm) {
                    (<kendo.ui.ContextMenu>$cm).close($(this));
                }
            });
        }, 500);
    }
...
function init(wg: Wizardsgroup) {
    $('body').addClass(<any>wg.window.browser).wg('browser', wg.window.browser);
    ...
    $document.on('click', 'a[data-wg-ui="kendoTabStrip"], button[data-wg-ui="kendoTabStrip"]', function (e) {
        e.preventDefault();
        let $tab: Wizardsgroup.UI.Kendo.TabStrip = wg.tab;
        if (self != top) {
            if ($(this).data('tab')) {
    ...
    $document.on('click', 'a[data-wg-ui="kendoGrid"]', function (e) {
        e.preventDefault();
        let $grid = {
            name: $(this).data('grid'), id: $(this).data('uid'), container: <JQuery>null
        };
    ...
    $document.on('click', '.k-grid .k-pager-refresh', function (e) {
        e.preventDefault();
        let $jquery = $(this).parents('.k-grid');
        if ($jquery) {
            var $grid = wg($jquery).grid();
            $grid.uncheckRecords();
            trigger('grid-' + $grid.name + '-refresh', $grid,
                'wg.bind.grid(\'' + $grid.name + '\').refresh(function(grid) {})');
        }
    });
    ...
    $document.on('click', 'a[data-contextmenu]', function (e) {
        e.preventDefault();
        let $this = $(this);
        let $jquery = $('#' + $(this).data('contextmenu'));
        let $contextmenu = wg($jquery).contextmenu();
...
class Modal {
    private data: Object;
    constructor(private jquery: JQuery) {
        let $id = this.jquery.data('uid');
        if ($id) {
            this.data = {
                id: $id
            }
    ...
    add(wg: Wizardsgroup) {
        ...
        wg.modal(c => c
            ...
            modal.block();
            wg.http.get(jquery.url(this.data),
                {
    ...
    delete(wg: Wizardsgroup) {
        let jquery = this.jquery;
        let $records = [];
        let $name = jquery.parents('div[data-container="grid"]').find('div[data-grid]').first().data('grid');
        let $grid = wg('#' + $name).grid();
        if ($name) {
        ...
        if ($records.length > 0) {
            wg.modal(c => c
                ...
                modal.block();
                wg.http.post(jquery.url(this.data),
                    {
                        checkedRecords: $records,
                        parentId: $('#' + $name).data('parent')
                    })
    ...
    static displayGrid(wg: Wizardsgroup, modal: Wizardsgroup.UI.Bootstrap.Modal, data: any) {
        var $columns = _.map(_.keys(data[0]), (i) => {
            return {
